...
8. **Rapid-growth quantity:** $|\dot{\mathcal{C}}(t)|$ — used for the aggregate H2 peak-alignment test (see §5). At $N=1$, the Schmidt-rank-2 bound caps $\mathcal{C}$ at $\log 2$, so the series peaks in $\mathcal{C}$ are sparse; $|\dot{\mathcal{C}}|$ is the informative quantity. *(Source: PA-04; Stage 4 §3.)*
9. **Time-averaged complexity:** $\bar{\mathcal{C}}$ over the resolvable window (where $\sigma^2_{\text{intrinsic}} > 10^{-8}$). ~~Used as the scalar summary paired with $f_{\text{resolved}}(M)$ in the Stage 8 H3 scatter.~~ *(Source: PA-03; Stage 3 §6 forward.)* **Superseded 2026-04-15 (PA-05):** the Stage 8 synthesis no longer uses $\bar{\mathcal{C}}$ vs $f_{\text{resolved}}$ as the H3 scatter. Both quantities are retained as per-point reporting scalars but the H3 headline figure is replaced by the per-mode complementarity structure (see §6 Stage 8 PA-05 amendment and §5 PA-05 hypothesis reformulation).
10. **[PA-05, 2026-04-15] Per-mode H2 correlation:** $r_k \equiv r\bigl(\sigma^2_{\text{intrinsic}},\, |\dot{\mathcal{C}}^{(k)}(t)|\bigr)$ for each mode $k$, over the resolvable window, where $\mathcal{C}^{(k)}(t) = \log n_{\text{eff}}^{(k)}(t)$. This is the PA-05-endorsed H2 diagnostic: the aggregate $r$ washes out the signal at $N \geq 2$ because weakly-coupled outer modes contribute uncorrelated variance to $|\dot{\mathcal{C}}|$ (Stages 6 B4 / 7). Per-mode $r_k$ for the dominantly-coupled (innermost) mode is consistently positive across Stages 4–7 at the inner-mode magnitude ($r_{\text{mode 1}} \in [+0.18, +0.55]$); this is the H2 signal. *(Source: PA-05; Stage 6 Gate 13 / Stage 7 Finding 2.)*
11. **[PA-05, 2026-04-15] Complementarity-pair characterisation:** the pair $(\mathcal{C}(t), \sigma^2_{\text{intrinsic}}(t))$ is reported as a joint trajectory in complexity-variance phase space rather than reduced to a scalar correlation. Per-moment peak-offset $\tau_k$ (time lag between nearest peaks of $\mathcal{C}^{(k)}$ and $\sigma^2$) records the complementarity structure, with the expected pattern of negative series-direct $r$ and positive growth-framing $r_k$ for the dominantly-coupled mode. *(Source: PA-05; Stage 4/6 series-direct negative $r$ finding; Stage 7 §5.2.)*
...
- ~~**H2 (moderate; growth-framing).** Peaks of $|\dot{\mathcal{C}}(t)|$ — the *moments of rapid complexity growth* — align temporally with peaks of $\sigma^2_{\text{intrinsic}}(t)$. Specifically, nearest-neighbour peak lag is small relative to the mean peak spacing, and Pearson $r\bigl(\sigma^2_{\text{intrinsic}},\,|\dot{\mathcal{C}}|\bigr) > 0$.~~
  ...
  **Superseded 2026-04-15 (PA-05).** The aggregate-growth-framing H2 holds at $N = 1$ across Cut A ($r$ rising monotonically toward resonance, Stage 5) and in 3/4 $N = 2$ configurations (Stage 6), but **breaks in the aggregate at $N = 3$ across all Cut C configurations** ($r \in [-0.18, +0.12]$; Stage 7 Finding 1). The underlying physics survives: **per-mode** growth-framing $r_k \equiv r(\sigma^2,\, |\dot{\mathcal{C}}^{(k)}|)$ is consistently positive for the dominantly-coupled mode at all $N$ and all configurations tested ($r_{\text{mode 1}} \in [+0.18, +0.55]$; Stage 7 Finding 2). The aggregate dilutes because weakly-coupled outer modes contribute uncorrelated variance to $|\dot{\mathcal{C}}|$. **Reformulated H2 (PA-05):** for each mode $k$, $r_k > 0$ when mode $k$ co-drives both $\sigma^2_{\text{intrinsic}}$ and $|\dot{\mathcal{C}}^{(k)}|$. The aggregate $r$ is meaningful only at $N = 1$ or near-symmetric coupling; at $N \geq 2$ with asymmetric coupling the aggregate is dominated by mode-decoupling artefacts (Stage 6 B4, Stage 7 Cut C). *(Source: PA-05; Stages 4–7.)*
...
- ~~**H3 (strong, Ordinans-adjacent).** Across the detuning sweep, $f_{\text{resolved}}(M)$ and $\bar{\mathcal{C}}$ co-vary monotonically: at the sweep's outer, far-from-resonance points, both are *low* (dynamics offers little to resolve; spin-mode entanglement stays close to product); at the inner, near-resonance points (the $|\Delta|/\omega_{\text{ref}} = 0.15$ edge of the accessible region), both are *high*. The claim is a *limit* claim, probed by fitting a trend across the three per-sign points $|\Delta|/\omega_{\text{ref}} \in \{0.15, 0.3, 0.5\}$ and extrapolating toward zero — not a point-claim at $\Delta = 0$ exactly (which is inaccessible in this voyage, §2.4 and Standing Q 12).~~
  ...
  **Superseded 2026-04-15 (PA-05) — honest null plus positive replacement.** H3 in its scalar-correlation form is **empirically falsified** across the voyage: $\bar{\mathcal{C}}$ and $f_{\text{resolved}}(M)$ do *not* co-vary monotonically across the sweep. Stage 5 showed non-monotonicity at $N = 1$ with a window-vs-period mechanism; Stages 6/7 showed it persists at $N = 2, 3$ with a deeper beat-frequency mechanism — $f_{\text{resolved}}$ tracks oscillation-cycle count, $\bar{\mathcal{C}}$ tracks resonance proximity and multi-mode participation; these are independent drivers. No single-scalar rescaling reconciles them.

  **Reformulated H3 (PA-05) — complementarity, not correlation.** Intrinsic complexity ($\mathcal{C}$) and operational variance ($\sigma^2_{\text{intrinsic}}$) probe **complementary** moments of the dynamics. $\mathcal{C}(t)$ peaks at maximum entanglement — the spin-reduced state at $p \approx 1/2$ — where $(\partial_\Delta p)^2$ is small, so $\sigma^2_{\text{intrinsic}}$ is small there. $\sigma^2_{\text{intrinsic}}(t)$ peaks at maximum parameter sensitivity — state-transition moments where $(\partial_\Delta p)^2$ is large — which is precisely when the spin-reduced state is near-pure and $\mathcal{C}$ is low. The empirical signatures: (i) series-direct $r(\sigma^2, \mathcal{C}) < 0$ consistently across Stages 4–7; (ii) growth-framing $r_k > 0$ for the dominantly-coupled mode; (iii) non-monotone $\bar{\mathcal{C}}$–$f_{\text{resolved}}$ scatter across configurations.

  **Scope of the complementarity claim.** This is a finding at $g/\omega_{\text{ref}} = 0.1$, $|\Delta|/\omega_{\text{ref}} \in [0.15, 0.5]$, $|\uparrow\rangle|0\rangle$ initial state, $N \leq 3$, closed-system dynamics. It is *not* claimed as a universal physics result; wider scope is a follow-up voyage. The speculative quantitative uncertainty-relation test $\mathcal{C}(t) \cdot \sigma^2(t)$ returned null at Stage 7 (peak product varies 5× across configs at $D=0.15$; no universal lower bound), so the complementarity stays **structural-descriptive**, not inequality-bounded. *(Source: PA-05; Stages 4–7 convergent evidence; Stage 7 §5.5 null on product test.)*

**Voyage real target after PA-05.** The central empirical content is the **per-mode complementarity structure**: intrinsic and operational measures probe conjugate dynamical aspects, their cross-walk reveals the per-mode structure of system-environment coupling, and the QFI-reduction boundary (Stage 5 Finding 3 / Stage 6 §6) maps where the first-order Fisher linearisation fails. H1 remains sanity. H2 (per-mode) is suggestive; H3 (complementarity) is the signature worth bringing to the MCTDH-numerics and open-systems collaborators.
...
- ~~**H3 scatter plot (headline figure):** $f_{\text{resolved}}(M = 1000)$ vs $\bar{\mathcal{C}}$ across all Cut A / B / C parameter points. $T_{\text{det}}$ and $t_{\text{rise}}$ as diagnostic side panels.~~ *(Source: PA-03; the H3 redefinition displaces v0.1's $T_{\text{det}}$-vs-saturation-$\mathcal{C}$ plot.)*
  ...
  **Superseded 2026-04-15 (PA-05) — headline figure redefined.** The Stage 8 headline is no longer the $f_{\text{resolved}}$-vs-$\bar{\mathcal{C}}$ scatter (which would show empirical non-monotonicity with no clean trend; Stages 5–7). It is the **per-mode complementarity structure across $N = 1, 2, 3$**: for each cut, show the joint $(\mathcal{C}^{(k)}(t), \sigma^2_{\text{intrinsic}}(t))$ phase-space trajectory of the dominantly-coupled mode plus the per-mode growth-framing correlation $r_k$ across configurations, with an auxiliary scatter recording $\bar{\mathcal{C}}$ and $f_{\text{resolved}}$ as separate reporting scalars (not as a correlation claim). *(Source: PA-05.)*
...
- **One-page writeup:** which hypotheses survived, parameter values, outcomes. The writeup must:
  ...
  - **[PA-05] Contract C5 reduction-path 3 *regionally*, not uniformly:** QFI reduction confirmed at outer single-mode configurations (Stage 3 at $|\Delta|=0.15$ at 7% median; Stage 5 at $|\Delta|=0.3, 0.5$ at ~10–13%) and breaks at inner single-mode ($|\Delta|=0.15$ at 22% in Cut A sweep) and across all $N=2$ (Stage 6, 13–30%) and $N=3$ (Stage 7) multi-mode configurations. The operational-crosswalk novelty survives in those regions; the platform-specific QFI restatement contracts only at outer single-mode.
  - **[PA-05] Report H2 per-mode, not aggregate.** State the aggregate-H2 null at $N=3$ and the per-mode positive finding (four-stage convergent). Aggregate washing is a mode-decoupling artefact, not an H2 failure.
  - **[PA-05] Retire H3-as-correlation; report H3 as complementarity.** Present the null on scalar correlation across sweeps honestly, then the complementarity structure as the positive replacement. Do *not* frame this as "we predicted complementarity all along" — the voyage predicted correlation; the data showed complementarity; that is an honest null-plus-positive trajectory and must read that way.
  - Report the Stage 6 Gate 12 / Stage 7 §5.4 IPR verdict alongside the $n_{\text{eff}}$ claim (IPR·$n_{\text{eff}} \in [1.00, 1.14]$ across all configs at $N = 2, 3$ — C3.4 licensed).
  - Report the Stage 7 commensurability comparison as a **clean null**: the three regimes do not cleanly separate in the aggregate scalars at the tested scope; the Ordinans-adjacent ordering signature is a follow-up-voyage question, not a Stage 7 conclusion. Do not hedge the null.
  - **[PA-05] Report the speculative $\mathcal{C}\cdot\sigma^2$ product test null** (Stage 7 §5.5). Complementarity is structural-descriptive, not an uncertainty-relation inequality.
  - Record scope caveats: Stage 7 at $R = 30$ / $n_{\max} = 10$ (qualitative findings robust, quantitative claims $\pm$ 20%); no noise-spectrum (coherent-modulation) cut run (Standing item 9 open); Krylov infrastructure deferred; BLP pair optimisation deferred.
- **Collaborator-handoff notes:** one for MCTDH-numerics (bounded MCTDH scenario), one for open-systems (non-Markovianity witness question, now framed against the Stage 3 BLP result and the Stage 5–7 complementarity finding).
